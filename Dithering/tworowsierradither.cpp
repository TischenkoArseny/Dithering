...
void TwoRowSierraDither::execute(QImage &img, palette &plt)
{
	...
	for (unsigned short y = 0; y < img.height(); ++y)
	{
		for (unsigned short x = 0; x < img.width(); ++x)
		{
			...
			if (x + 2 < img.width())
			{
                ...
                offsetPixel.setRed((static_cast<int>(offsetPixel.getRed()) + ((redError << 2) >> 4)));
                offsetPixel.setGreen((static_cast<int>(offsetPixel.getGreen()) + ((greenError << 2) >> 4)));
                offsetPixel.setBlue((static_cast<int>(offsetPixel.getBlue()) + ((blueError << 2) >> 4)));
			...
			if (y + 1 < img.height())
			{
				...
				if (x - 1 >= 0)
				{
                    ...
                    offsetPixel.setRed((static_cast<int>(offsetPixel.getRed()) + ((redError <<1 )>> 4)));
                    offsetPixel.setGreen((static_cast<int>(offsetPixel.getGreen()) + ((greenError << 1) >> 4)));
                    offsetPixel.setBlue((static_cast<int>(offsetPixel.getBlue()) + ((blueError << 1 ) >> 4)));
