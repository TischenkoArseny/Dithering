...
void AtkinsonDither::execute(QImage &img, palette &plt)
{
	...
	for (unsigned short y = 0; y < img.height(); ++y)
	{
		for (unsigned short x = 0; x < img.width(); ++x)
		{
			...
			if (x + 2 < img.width())
			{
                ...
                offsetPixel.setRed((static_cast<int>(offsetPixel.getRed()) + (redError >> 3)));
                offsetPixel.setGreen((static_cast<int>(offsetPixel.getGreen()) + (greenError >> 3)));
                offsetPixel.setBlue((static_cast<int>(offsetPixel.getBlue()) + (blueError >> 3)));
